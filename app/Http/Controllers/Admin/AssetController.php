...
class AssetController extends Controller
{
    ...
    public function index()
    {
        ...
        $assets = Asset::with(['category', 'status', 'location', 'media'])->get();
    ...
    public function create()
    {
        ...
        return view('admin.assets.create', compact('categories', 'locations', 'statuses'));
    }
    ...
    public function edit(Asset $asset)
    {
        ...
        $asset->load('category', 'status', 'location');

        return view('admin.assets.edit', compact('asset', 'categories', 'locations', 'statuses'));
    }

    public function update(UpdateAssetRequest $request, Asset $asset)
    {
        $attr = $request->all();
        $attr['updated_at'] = now('Asia/Jakarta');
        $asset->update($attr);
    ...
    public function show(Asset $asset)
    {
        ...
        $data = implode("\n", [
            'Kode Barang' . '= ' . $asset->serial_number,
            'Nama Asset' . '= ' . $asset->name,
            'Merk/Type' . '= ' . $asset->merk,
            'Tahun Pembelian' . '= ' . $asset->pemebelian,
            'Harga' . '= ' . $asset->harga,
            'Lokasi' . '= ' . $asset->location->name,
            'Sub Lokasi' . '= ' . $asset->sub_location,
            'Lantai' . '= ' . $asset->lantai,
            'Penanggung Jawab' . '= ' . $asset->penanggung_jawab,
            'Status Kondisi' . '= ' . $asset->status->name
        ]);
        $asset->load('category', 'status', 'location');
        $qrCode = QrCode::size(128)->generate($data);
    ...
    {
        // Ambil data yang akan di-generate ke dalam QR code
        $data = implode("\n", [
            'Kode Barang' . '= ' . $asset->serial_number,
            'Nama Asset' . '= ' . $asset->name,
            'Merk/Type' . '= ' . $asset->merk,
            'Tahun Pembelian' . '= ' . $asset->tahun,
            'Harga' . '= ' . $asset->harga,
            'Lokasi' . '= ' . $asset->location->name,
            'Sub Lokasi' . '= ' . $asset->sub_location,
            'Lantai' . '= ' . $asset->lantai,
            'Penanggung Jawab' . '= ' . $asset->penangguna_jawab,
            'Status Kondisi' . '= ' . $asset->status->name
        ]);

        // Generate QR code dengan menggunakan library qrcode
        $qrCode = QrCode::size(250)->generate($data);

        // Simpan QR code ke dalam file PNG
        $filename = 'qr-code' . $asset->serial_number .'.svg';
        $path = storage_path('app/public/qrcode/' . $filename);
        QrCode::format('svg')->size(250)->generate($data, $path);
